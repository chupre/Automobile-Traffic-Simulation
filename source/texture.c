...
unsigned int carTexture, backgroundTexture, redLight, greenLight, yellowLight;
...
void loadTexture(char *image_name, unsigned int *texture) {
  ...
  texture_pixels = stbi_load(full_path, &texture_width, &texture_height,
                             &texture_channels, 0);
  GLenum format;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, format, texture_width, texture_height, 0,
               format, GL_UNSIGNED_BYTE, texture_pixels);
  glGenerateMipmap(GL_TEXTURE_2D);

  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                  format == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_MIRRORED_REPEAT);
